<?php
/**
 * Fichier db.php
 *
 * LICENCE
 *     You are free:
 *       to Share - to copy, distribute and transmit the work
 *       to Remix - to adapt the work
 *     Under the following conditions:
 *       Attribution - You must attribute the work in the manner specified by the author or licensor 
 *       (but not in any way that suggests that they endorse you or your use of the work).
 *       Just keep this header.
 *
 * @copyright  2008 Wamania.com
 * @license    http://creativecommons.org/licenses/by/2.0/fr/
 * @version    $Id:$
 * @link       http://www.wamania.com
 * @since      File available since Release 0.1
*/

/**
 * Classe d'accès à la BDD, utilise PDO
 * 
 * Peut être utiliser indépendemment après initialisation de Lithium
 * <code>
 * $db = LiDb::getSingleton();
 * $results = $db->query("SELECT ... FROM ...");
 * foreach ($results as $result) {
 *     echo $result->id;
 * }
 * </code>
 *
 *
 * @copyright  2008 Wamania.com
 * @license    http://creativecommons.org/licenses/by/2.0/fr/
 * @package    Lithium
 * @subpackage ORM
 * @version    Release: @package_version@
 * @link       http://www.wamania.com
 * @since      Class available since Release 0.1
 */
class LiOrmDb
{
    /**
     * Connexion DSN
     * @var String
     */
    private static $dsn;
    
    /**
     * Connexion user
     * @var String
     */
    private static $user;
    
    /**
     * Connexion pass
     * @var String
     */
    private static $pass;
    
    /**
     * @var Object of PDO
     */
    private $pdo;
    
    /**
     * @var Object of PDOStatement
     */
    private $pdoStatement;
    
    /**
     * Singleton
     * @var Object of LiDb
     */
    private static $instance;
    
    /**
     * Get unique instance
     * @return 
     */
    public static function getSingleton() 
    {
        if ( ! isset(self::$instance)) {
            self::$instance = new liDb;
        }
        return self::$instance;
    }
    
    /**
     * Set connexion config
     * @return void
     * @param Array $config
     */
    public static function setConfig($config) 
    {
        self::$dsn = $config['dsn'];
        self::$user = $config['user'];
        self::$pass = $config['pass'];
    }
    
    /**
     * Create PDO Object
     * @return void
     * @throw LiException
     */
    public function connect() 
    {
        if ( (empty(self::$dsn)) || (empty(self::$user)) || (empty(self::$pass)) ) {
            throw new LiException('You must give the access codes to the data base');
        }

        try {
            $this->pdo = new PDO(self::$dsn, self::$user, self::$pass);
            
            $this->pdo->exec("SET NAMES 'utf8'");
        	$this->pdo->exec("SET CHARACTER SET 'utf8'");
        	
        	$this->pdo->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
        
        } catch(PDOExcetpion $e) {
            //throw new LiException($e->getMessage());
            echo 'Echec connexion PDO : '.$e->getMessage()."\n<br />";
        }
    }
    
    /**
     * Destroy PDO Object
     * @return void
     */
    public function disconnect() 
    {
        $this->pdo = null;
    }
    
    /**
     * Query database
     * @return void
     * @param String $query
     * @param Array $params[optional]
     */
    public function query($query, $params=array()) 
    {
        if ($this->pdo === null) {
            $this->connect();
        }

        $this->pdoStatement = $this->pdo->prepare($query);
        $this->pdoStatement->setFetchMode(PDO::FETCH_ASSOC);
        try {
            $this->pdoStatement->execute($params);
        } catch(PDOException $e) {
            echo 'Echec requete : '.$e->getMessage()."\n<br />".$query;
        }
    }
    
    /**
     * Get last insert ID
     * @return Integer
     */
    public function lastId() 
    {
        return $this->pdo->lastInsertId();
    }
    
    /**
     * Fetch all datas
     * @return Array
     */
    public function fetchAll() 
    {
        return $this->pdoStatement->fetchAll();
    }
    
    /**
     * Get num rows (affected or selected)
     * @return Integer
     */
    public function numRows() 
    {
        return $this->pdoStatement->rowCount();
    }
    
    public function &getPdo()
    {
    	if ($this->pdo === null) {
            $this->connect();
        }
    	return $this->pdo;
    }
}
